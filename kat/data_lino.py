...
class KATDataset(Dataset):
    def __init__(
        ...
        # elif cecap != False and type(cecap) != List:
        #     if 'cecap_reddit' in cecap:
        #         data_file = os.path.join(data_dir, cecap)
        #     else:
        #         data_file = os.path.join(data_dir, cecap+'.data')
        elif cecap != False and type(cecap) == list:
            multiple_data = True
            self.examples = []
            for c in cecap:
                ...
                if 'cecap_reddit' in c:
                    data_file = os.path.join(data_file, c)
                else:
                    data_file = os.path.join(data_file, c + '.data')
...
class KATDataCollator:
    ...
    def _encode(self, batch:List[Example], shuffle_kno=True, include_true_kno=False, sort_kno=False, is_kat_tslf=True) -> Dict[str, torch.Tensor]:  # Lino version
            ...
            if shuffle_kno and include_true_kno:
                ...
                if is_kat_tslf:
                    if len(kno) < max_num_kno:
                        kno_text = kno_text + ['pad'] * (max_num_kno - len(kno))  # list 에 pad 추가
                    else:
                        kno_text = kno
                else:
                    for jj in range(j+1):
        ...
        #     max_target_length=self.data_1args.max_target_length,
        #     padding="longest",
        #     return_tensors="pt",
        # ).data
        input_texts = [[st, kt] for st, kt in zip(src_texts, kno_texts)]

        # #TODO: knowledge 를 하나씩 encode 시켜가면서 1024 길이 넘어가는 때를 포착하고,
        # #TODO: 그 이후의 knowledge 들은 삭제시켜준다.
        # maxl = self.data_args.max_source_length
        # true_knowledges = []  # for checking purpose
        # for i, (st, kt) in enumerate(input_texts):
        #     tkn_st = self.tokenizer(st, padding=False, truncation=False)
        #     cur_len = len(tkn_st['input_ids'])
        #     true_knowledges.append(kt[0])
        #     for j in range(len(kt)):
        #         tkn_kt = self.tokenizer(kt[j], padding=False, truncation=False)
        #         cur_len += len(tkn_kt)
        #         if cur_len > maxl:
        #             # Randomly select one position to put the true knowledge.
        #             # The true knowledge is set not to be truncated.
        #             true_kno_position = random.sample(range(j), k=1)
        #             # swap knowledge -- cf) input_texts[i][1][0]: ith example in a batch, 1th entry for knowledge, 0 for truth knowledge.
        #             input_texts[i][1][0], input_texts[i][1][true_kno_position] = \
        #                 input_texts[i][1][true_kno_position], input_texts[i][1][0]
        #             break
