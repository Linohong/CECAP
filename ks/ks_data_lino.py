...
class KATDataCollator:
    ...
    def __call__(self, batch) -> Dict[str, torch.Tensor]:
        ...
        if batch_elems[3] == []:  # is_train case.
            batch = {
                "input_ids": batch_elems[0],
                "attention_mask": batch_elems[1],
                "labels": batch_elems[2],
            }
        else:  # is evalution case.
            batch = {
                "input_ids": batch_elems[0],
                "attention_mask": batch_elems[1],
                "labels": batch_elems[2],
                "eval_ids": batch_elems[3]
            }
        return batch

    def _encode(self, batch:List[Example], shuffle_kno=True, include_true_kno=False, sort_kno=False) -> Dict[str, torch.Tensor]:  # Lino version
        src_texts = [x.src for x in batch]
        cand_knowls = [x.cand_knowl for x in batch]
        labels = [0 if x.label == True else 1 for x in batch]
        eval_dial_ids = []

        if batch[0].is_train == False:
            for x in batch:
                if x.src in self.eval_dialogue_id:
                    eval_dial_ids.append(self.eval_dialogue_id[x.src])
                else:
                    assign_id = self.eval_dialogue_id['__id__']
                    self.eval_dialogue_id[x.src] = assign_id
                    self.eval_dialogue_id['__id__'] = assign_id + 1
                    eval_dial_ids.append(assign_id)
        ...
        return input_ids, attention_mask, labels, eval_dial_ids
